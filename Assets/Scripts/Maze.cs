...
public class Maze : MonoBehaviour
{
    ...
    private bool startChosen = false, endChosen = false;
    ...
    void Start()
    {
        ...
        d_tree = dijkstra.Dijkstra_Solve(adjacencyMatrix, allNodes.Count, start, finish); // change for after testing purposes
    ...
    IEnumerator CheckNodes()
    {
        ...
        for (int i = 0; i < adjacencyMatrix.GetLength(0); i++)
        {
            for (int j = 0; j < adjacencyMatrix.GetLength(0); j++)
            {
                ...
                {
                    allNodes[i].cellFloor.GetComponent<Renderer>().material.color = Color.red;
                    allNodes[j].cellFloor.GetComponent<Renderer>().material.color = Color.red;
                    yield return new WaitForSeconds(debugFlashWaitTime);
                    allNodes[i].cellFloor.GetComponent<Renderer>().material.color = new Color32(7, 0, 203, 255);
                    allNodes[j].cellFloor.GetComponent<Renderer>().material.color = new Color32(7, 0, 203, 255);
                    yield return new WaitForSeconds(debugFlashWaitTime);
            ...
            if (allNodes[i].cellFloor.GetComponent<Text>() == null)
            {
                var text = allNodes[i].cellFloor.AddComponent<Text>();
    ...
    void LabelNodes()
    {
        ...
        for (int i = 0; i < cells.Length; i++)
        {
               ...
               && currCell.east == null && currCell.west == null))
            {
                ...
                currCell.cellFloor.GetComponent<Renderer>().material.color = Color.cyan;
            } else // Raise cell floors slightly
            {
                currCell.cellFloor.transform.Translate(0.0f, 0.15f, 0.0f);
                
            }
        }
    }

    void CreateAdjacencyMatrix() // Reminder: REFACTOR
    {
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (clicked.transform && startChosen != true)
                {
                    ChooseEndpoint(clicked.transform);
                    startChosen = true;
                } else if (clicked.transform && !endChosen)
                {
                    ChooseEndpoint(clicked.transform);
                    endChosen = true;
                }
        ...
        //{
        //    if (hitInfo.transform && startChosen != true)
        //    {
        //        ChooseEndpoint(hitInfo.transform);
        //        startChosen = true;
        //    }
        //    else if (hitInfo.transform && !endChosen)
        //    {
        //        ChooseEndpoint(hitInfo.transform);
        //        endChosen = true;
        //    }
